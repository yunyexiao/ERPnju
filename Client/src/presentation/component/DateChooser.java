...
public class DateChooser extends JPanel{
    ...
    private void commit() {
        if (showDate instanceof JTextField) {
        	...
        	Matcher m = p.matcher(((JButton) showDate).getText());
        	m.find();
        	String text = m.group(0);
        	((JButton) showDate).setText("<html><center>" + text + "</center></p><p style=\"line-height:0px\">" + sdf.format(calendar.getTime()) + "</p></html>");
        }
        hidePanel();
    }
